...
function getJourneySummaryLocations(journey: Journey) {
  if (journey.locations.length > 0) {
    return journey.locations;
  }

  const fallbackLocations = [journey.departure, journey.destination]
    .filter((location): location is JourneyLocation => Boolean(location));

  return fallbackLocations.filter((location, index, collection) => (
...
function JourneyCard({
  ...
  const summaryLocations = getJourneySummaryLocations(journey);
  const isMultiStopJourney = journey.locations.length > 2;
  const usesMinimalArchiveLayout = !isFlightTicket && isDefaultLayout && summaryLocations.length <= 1;
  ...
  const locationSummary = joinLocationLabels(summaryLocations);
