...
import type { JourneyLocation } from './travelConfig';
...
// --- Chinese provinces ---
const PROVINCE_GEOJSON_MAP: Record<string, string> = {
...
const PROVINCES: LocationOption[] = CHINA_PROVINCES.map(p => ({
  type: 'province',
  name: PROVINCE_GEOJSON_MAP[p.name] ?? p.name + '省',
  label: p.name,
...
export const ALL_LOCATIONS: LocationOption[] = [...COUNTRIES, ...PROVINCES, ...CITIES];
