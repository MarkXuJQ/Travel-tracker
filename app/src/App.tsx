...
const decodeShareData = (encoded: string): unknown | null => {
  try {
...
function App() {
  ...
  const statistics = useMemo(
    () => getStatistics(),
    [getStatistics, records, visited],
  );
  const visitedCities = useMemo(
    () => getVisitedCities(),
    [getVisitedCities, visited],
  );
  const visitedProvinces = useMemo(
    () => getVisitedProvinces().map((p) => p.name),
    [getVisitedProvinces, visited],
  );
  const visitedCountries = useMemo(
    () => getVisitedCountries().map((c) => c.name),
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (shareData) {
        ...
        if (isObject(decoded) && Array.isArray(decoded.records)) {
    ...
    >
      ...
      {isDefaultView && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-40">
          ...
          <div className="text-center mb-8">
            <h1
              className="text-5xl md:text-7xl font-bold text-[#f0f2ff] mb-4"
              style={{ textShadow: "0 0 40px rgba(76,201,240,0.5)" }}
            >
              旅行轨迹星球
            </h1>
          ...
          <div className="flex gap-4 md:gap-8 mb-8">
            <div className="bg-[rgba(26,31,61,0.9)] backdrop-blur-md border border-[rgba(247,37,133,0.3)] rounded-2xl px-6 py-4 text-center">
              <p className="text-3xl md:text-4xl font-bold text-[#f72585]">
                {statistics.global.visited}
              </p>
            ...
            <div className="bg-[rgba(26,31,61,0.9)] backdrop-blur-md border border-[rgba(76,201,240,0.3)] rounded-2xl px-6 py-4 text-center">
              <p className="text-3xl md:text-4xl font-bold text-[#4cc9f0]">
                {statistics.provinces.visited}
              </p>
      ...
      {!isDefaultView && !isMobile && (
        ...
        >
          <div className="bg-[rgba(26,31,61,0.8)] backdrop-blur-md border border-[rgba(247,37,133,0.3)] rounded-full px-4 py-2 flex items-center gap-2">
            ...
            <span className="text-[#f0f2ff] text-sm">
              {statistics.global.visited} 个国家
            </span>
          </div>
          <div className="bg-[rgba(26,31,61,0.8)] backdrop-blur-md border border-[rgba(76,201,240,0.3)] rounded-full px-4 py-2 flex items-center gap-2">
            ...
            <span className="text-[#f0f2ff] text-sm">
              {statistics.provinces.visited} 个省份
            </span>
          </div>
      ...
      {!isDefaultView && !isMobile && (
        ...
        >
          ...
          <span className="text-[#f0f2ff]/30 text-xs">
            拖动地球旋转 · 滚轮缩放 · 点击板块查看
          </span>
        </div>
      )}
      ...
      {!isDefaultView && isMobile && (
        <div className="fixed bottom-4 left-4 right-4 z-40">
          <div className="bg-[rgba(26,31,61,0.9)] backdrop-blur-md rounded-full px-4 py-2 text-center">
            <span className="text-[#f0f2ff]/50 text-xs">
              拖动旋转 · 双指缩放 · 点击点亮
            </span>
          </div>
        </div>
        ...
        <DialogContent className="bg-[rgba(26,31,61,0.95)] border-[rgba(76,201,240,0.3)] max-w-sm">
          ...
          <div className="flex gap-3 mt-4">
            <Button
              ...
              className="flex-1 border-[rgba(76,201,240,0.3)] text-[#f0f2ff]"
            >
              取消
            </Button>
            <Button
              onClick={handleImport}
              className="flex-1 bg-[#4cc9f0] hover:bg-[#4cc9f0]/80 text-white"
            >
              导入
            </Button>
          </div>
